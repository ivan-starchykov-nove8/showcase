...
class FeatureDNumberButtonsWidget extends StatelessWidget {
  const FeatureDNumberButtonsWidget({super.key});

  @override
  Widget build(BuildContext context) {
    return Column(
      children: List.generate(3, (index) {
        final int number = index + 4;
        return Padding(
          padding: const EdgeInsets.symmetric(vertical: 4.0),
          child: DefaultCommonButton(
            onPressed: () {
              context.read<FeatureBBloc>().add(StepTwoNumberSelected(number));
            },
